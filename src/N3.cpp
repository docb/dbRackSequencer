  ...
  void process(const ProcessArgs &args) override {
    ...
    if(paramDivider.process()) {
      if(inputs[DEGREE_INPUT].isConnected()) {
        getParamQuantity(DEGREE_PARAM)->setValue(inputs[DEGREE_INPUT].getVoltage()*9);
      }
      for(int k=0;k<NUM_SEQ;k++) {
        if(inputs[ON_INPUT+k].isConnected()) {
          getParamQuantity(ON_PARAM+k)->setValue(inputs[ON_INPUT+k].getVoltage()>0.f?1.f:0.f);
        }
        if(inputs[VERTICES_INPUT+k].isConnected()) {
          getParamQuantity(VERTICES_PARAM+k)->setValue(inputs[VERTICES_INPUT+k].getVoltage()*3.2f);
        }
        if(inputs[SKEW_INPUT+k].isConnected()) {
          getParamQuantity(SKEW_PARAM+k)->setValue(inputs[SKEW_INPUT+k].getVoltage()*36);
        }
        if(inputs[ROT_INPUT+k].isConnected()) {
          getParamQuantity(ROT_PARAM+k)->setValue(inputs[ROT_INPUT+k].getVoltage()*36);
        }
